...
def parse_args():
    ...
    p.add_argument("--data_dir", default="./val_rest", help="目录里包含 corpus.jsonl / test_queries.jsonl / test_qrels.tsv")
...
def embed_text_batch(texts: List[str], model, batch_size=64):
...
def main():
    ...
    if collection.count() == 0:
        ...
        for i in tqdm(range(0, len(docs), args.batch_size), desc="Indexing"):
            ...
            batch_embs = embed_text_batch(batch_texts, model)
    ...
    for i in tqdm(range(0, len(queries), args.batch_size), desc="Evaluating"):
        ...
        q_embs = embed_text_batch(texts, model)
